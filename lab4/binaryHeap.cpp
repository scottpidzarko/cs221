...
void printHeap(int *heap, int size, int node=0, int d=0) {
  ...
  //Assume Heap is sorted
  if (printQ1Style == true) {
    if (size == 0) {
      std::cout << "Input Heap of Size 0" << std::endl;
      return;
    }
    ...
    //printing in style according to Q1 (tree-like, vertical)
    ...
    for(int i = 1; i < size;, i = i +(2*i)){
      ...
      std::cout << leftChild << std::end;
    }



    //print a newline to make stuff to look nice (lol)
    std::cout << std::endl;

  }else{
    //printing in style according to Q2 (tree-like, 90 degrees rotated to horizontal)
  }
}
